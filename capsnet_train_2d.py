...
class TrainCapsNet2D:
    ...
    def __init__(self, saved_model_path=None):
        ...
        ###########################################################
        # Experiment notes:
        self.experiment = '2D CapsNet, slice rebalance (max 1)'
        ...
        self.results_folder = 'data/results/7.27.22_capsnet2d_3rdvent'
                ...
                for slc in slices:
                    ...
                    if targets[:, :, slc, :, :].sum() < 1:
                        ...
                        if self.slice_rebalance and slices_without_target > self.max_slices_without_target:
                            with torch.no_grad():
                                outputs[:, :, slc, :, :] = self.model(inputs[:, :, slc, :, :])
                            continue

                    self.optimizer.zero_grad()
                    output = self.model(inputs[:, :, slc, :, :])
                    # print('slice: ', slc)
