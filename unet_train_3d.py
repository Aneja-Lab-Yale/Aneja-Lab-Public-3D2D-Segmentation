...
from unet_model_3d import UNet3D
...
class TrainUNet3D:
    ...
    def __init__(self, saved_model_path=None):
        ...
        self.experiment = '3D U-Net, train120'

        # Set segmentation target:
        self.output_structure = '3rd ventricle'
        ...
        self.results_folder = 'data/results/22.06.19_unet3d_3rdvent_train120'
        ...
        self.train_outputs_csv = 'train_outputs_120.csv'
            ...
            for i, data_batch in enumerate(self.valid_dataloader):
                ...
                for slc in range(D):
                    if targets[:, :, slc, :, :].sum() > 0:
                        outputs_with_target[:, :, slc, :, :] = outputs[:, :, slc, :, :]
                    else:
                        outputs_without_target[:, :, slc, :, :] = outputs[:, :, slc, :, :]
    ...
    def save_niftis(self):
        ...
        for data_batch in tqdm(valid_dataloader, desc='Saving NIfTi files'):
            ...
            with torch.no_grad():
                outputs = self.model(inputs)
                batch_losses = self.dice_loss_no_reduction(outputs, targets)
